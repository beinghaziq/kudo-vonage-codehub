export const sourceLanguagesList = [
  { name: 'BRAZILIAN PORTUGUESE', code: 'PTB' },
  { name: 'CANADIAN FRENCH', code: 'FRC' },
  { name: 'BRITISH ENGLISH', code: 'ENGB' },
  { name: 'ENGLISH', code: 'ENG' },
  { name: 'FRENCH', code: 'FRE' },
  { name: 'GERMAN', code: 'GER' },
  { name: 'ITALIAN', code: 'ITA' },
  { name: 'MEXICAN SPANISH', code: 'ESM' },
  { name: 'PORTUGUESE', code: 'POR' },
  { name: 'SPANISH', code: 'SPA' },
];

export const predefinedTargetLanguagesList = [
  { value: 'HIN', label: 'HINDI' },
  { value: 'ENG', label: 'ENGLISH' },
  { value: 'SPA', label: 'SPANISH' },
  { value: 'POR', label: 'PORTUGUESE' },
  { value: 'FRE', label: 'FRENCH' },
  { value: 'ITA', label: 'ITALIAN' },
  { value: 'GER', label: 'GERMAN' },
  { value: 'CHI', label: 'CHINESE' },
  { value: 'CHT', label: 'CHINESE (TAIWAN)' },
];

export const targetLanguagesList = [
  { name: 'BRAZILIAN PORTUGUESE', code: 'PTB' },
  { name: 'CANADIAN FRENCH', code: 'FRC' },
  { name: 'BRITISH ENGLISH', code: 'ENGB' },
  { name: 'ENGLISH', code: 'ENG' },
  { name: 'FRENCH', code: 'FRE' },
  { name: 'GERMAN', code: 'GER' },
  { name: 'ITALIAN', code: 'ITA' },
  { name: 'MEXICAN SPANISH', code: 'ESM' },
  { name: 'PORTUGUESE', code: 'POR' },
  { name: 'SPANISH', code: 'SPA' },
  { name: 'ARABIC', code: 'ARA' },
  { name: 'CHINESE', code: 'CHI' },
  { name: 'CHINESE (TAIWAN)', code: 'CHT' },
  { name: 'CROATIAN', code: 'HRV' },
  { name: 'CZECH', code: 'CZE' },
  { name: 'DANISH', code: 'DAN' },
  { name: 'DUTCH', code: 'DUT' },
  { name: 'FINNISH', code: 'FIN' },
  { name: 'GREEK', code: 'GRK' },
  { name: 'HEBREW', code: 'HEB' },
  { name: 'HINDI', code: 'HIN' },
  { name: 'JAPANESE', code: 'JPN' },
  { name: 'KOREAN', code: 'KOR' },
  { name: 'NORWEGIAN', code: 'NOR' },
  { name: 'PERSIAN (FARSI)', code: 'PER' },
  { name: 'POLISH', code: 'POL' },
  { name: 'ROMANIAN', code: 'RUM' },
  { name: 'RUSSIAN', code: 'RUS' },
  { name: 'SWEDISH', code: 'SWE' },
  { name: 'TURKISH', code: 'TUR' },
  { name: 'UKRAINIAN', code: 'UKR' },
];
